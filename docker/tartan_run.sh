...
set_variables() {
    if [ "$option_devel" = "true" ]; then
        ...
        # Verify auotware data exists
        if [ ! -d "$AUTOWARE_DATA_PATH" ]; then
            echo "$AUTOWARE_DATA_PATH does not exist! Please download artifacts in $AUTOWARE_DATA_PATH first"
            exit 1
        fi
...
set_gpu_flag() {
    if [ "$option_no_nvidia" = "true" ]; then
        ...
        GPU_FLAG="--gpus all"
        IMAGE=${IMAGE}-cuda
        # IMAGE=${IMAGE}-cuda-20250110-amd64
    fi
}
